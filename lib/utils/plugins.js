...
function getPlugins() {
  if (!_plugins) {
    _plugins = remote.require('./plugins');
  }
  return _plugins;
}
...
const clearModulesCache = () => {
  ...
  const {path, localPath} = getPlugins().getBasePaths();
  ...
  for (const entry in require.cache) {
    if (entry.indexOf(path) === 0 || entry.indexOf(localPath) === 0) {
      // `require` is webpacks', `require` is electron's
...
const getPluginVersion = path => {
  let version = null;
  try {
    version = require(pathModule.resolve(path, 'package.json')).version;
  } catch (err) {
    //eslint-disable-next-line no-console
...
export const loadModules = () => {
  ...
  const paths = getPlugins().getPaths();
  ...
  console.log('DECORATORS', reducersDecorators);

  const loadedPlugins = getPlugins().getLoadedPluginVersions().map(plugin => plugin.name);
  ...
  const deprecatedPlugins = getPlugins().getDeprecatedConfig();
...
function decorateReducer(name, fn) {
  console.log('DECORATING', reducersDecorators);
